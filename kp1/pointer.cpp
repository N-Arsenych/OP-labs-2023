...
char* replace_operators(const char* line)
{
    ...
    char* result = new char[MAX_LINE_LEN];                                               // виділяємо пам'ять для нового рядка
    strncpy_s(result, (sizeof(result)+5) /* / sizeof(result[0])*/ , new_str.c_str(), new_str.size());      //копіюємо результат в масив result
    result[MAX_LINE_LEN - 1] = '\0';                                            // додаємо нуль-термінатор 

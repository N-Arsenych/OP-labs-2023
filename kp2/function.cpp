...
void read(string fname)
{
	...
	if (!in)
	{
		...
		Products* product = new Products[size];

		for (size_t i = 0; i < size; i++) {
			in.read((char*)&product[i], sizeof(Products));
			product[i].Print();
		}
...
void expiration(string fname, string fnew1, string fnew2) {
	...
	if (!in)
	{
		...
		for (size_t i = 0; i < size; i++) {
			...
			int m = 3;
			int d = 24;
			if (date(y, m, d) - date(p[i].e_year, p[i].e_month, p[i].e_day) < 0)
			{
				if (total_days <= 5)
				{
...
void operations(int ch, string fname, string fnew1, string fnew2) 
{
